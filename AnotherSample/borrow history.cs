    ...
    public partial class borrow_history : Form
    {
        ...
        private void LoadTransactionHistory()
        {
            try
            {
                ...
                // SQL query to get transaction data for the logged-in user, excluding records with NULL transaction_borrow_date
                string query = @"
            SELECT 
            ...
            WHERE t.transaction_user_id = @userId
              AND t.transaction_borrow_date IS NOT NULL;  -- Filter out records without a borrow date
        ";
